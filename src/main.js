...
import { scenes, total } from './scenes.js'
...
const sceneMarkup = (scene, i) => `
  ...
  >
    <div class="scene__inner">
      <div class="copy">
        ${scene.lockup
          ? `<div class="lockup">${logo('logo--lg', true)}<span class="lockup__claim">See what matters.</span></div>`
        ...
        ${scene.cta ? `<div class="copy__cta"><a class="btn btn--lg" href="https://temis.app" target="_blank" rel="noopener">${scene.cta.label} <i>&rarr;</i></a><small>${scene.cta.note}</small></div>` : ''}
      </div>
...
document.querySelector('#app').innerHTML = `
  ...
  <header class="topbar">
    ...
    <p class="topbar__claim">Más datos. Mejores decisiones. Negocios más fuertes.</p>
...
// The isotipo lives outside the scenes and flies to the slot each scene leaves for it,
// so the mark reads as one object travelling through the story.
...
// Anchors move with the layout, so the spark is re-parked whenever the box changes.
...
function paint(i) {
  ...
  document.body.dataset.tone = scene.tone
...
const fromHash = () => {
  ...
  const bySlug = scenes.findIndex(s => s.id === raw || `escena-${scenes.indexOf(s) + 1}` === raw)
  return bySlug >= 0 ? bySlug : 0
}

window.addEventListener('hashchange', () => go(fromHash()))

// Handle for inspecting deck state from the console during development.
